...
function StartInterview() {
  ...
  const GenerateFeedback = async () => {
    // Prevent empty conversation error 
    if (conversation === undefined) {
      ...
      console.log("Empty feedback sended .");
      ...
      try {
        ...
        console.error("JSON Parse Error:", err, FINAL_CONTENT);
        return;
      }

      const { error, data } = await supabase.from("interview-feedback").insert([
        {
          userName: interviewInfo?.userName,
          userEmail: interviewInfo?.userEmail,
          interview_id: interview_id,
  ...
  const startCall = () => {
    ...
    🔹 Wrapping Up:
    - After 5-6 questions, **summarize** their performance.
      Example:
  ...
  const stopInterview = () => {
    if (vapi.current) {
      ...
      setStartTimer(false);
      setResetTimer(true);
      toast.success("Interview has been manually stopped.");
